...
Thanks to the `reflex-metadata` library you can "hack" into the Typescript compiler and get the static
type descriptor from compile-time into your Javascript runtime.
...
and this metadata may affect your Javascript runtime code.
...
Let's check on how to define an SQL table via the `type-orm` library using decorators and `reflex-metadata`.
...
The default Typescript compiler removes information about data types. Thanks to `reflex-metadata` this code is still working
...
The `reflex-metadata` library has to influence the Typescript compiler and forces us to vendor lock our code into Typescript specific
syntax so we're no longer able to use raw vanilla Javascript.
The beauty of standard Typescript is that it just enhances the real Javascript codebase and enables us to have
...
We don't need to bend our application in the name of strict-type-safe-only faith.
The strongest type-system advantage of Typescript over the others is
...
real Javascript reference and static type reference. It mean that you do not follow SSOT (Single source of truth) and DRY (Don't repeat yourself) at all.
...
- A code that should be type-safe
- Good documentation
...
So we have explained why using `reflex-metadata` suc*s...so what should we use instead?
...
Then we can Infer the data types.
Thanks to that our code is pure Javascript, we're able to be more flexible and generate data types on the fly and not be fixed.
...
The problem is the authors of libraries who think that this **new** `reflex-metadata` API with experimental decorators will
save the world, but at the opposite side it just vendor locks your codebase into 1 edge-case technology.
